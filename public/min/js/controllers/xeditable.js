...
app.controller('XeditableCtrl', ['$scope', '$filter', '$http', 'editableOptions', 'editableThemes',
  function($scope, $filter, $http, editableOptions, editableThemes){
    ...
    // editable table
    $scope.users = [
      {id: 1, name: '张小三', status: 2, group: 4, groupName: '管理员'},
      {id: 2, name: '李四', status: undefined, group: 3, groupName: '高级会员'},
      {id: 3, name: '王老五', status: 2, group: null}
    ];

    $scope.groups = [];
    $scope.loadGroups = function() {
      return $scope.groups.length ? null : $http.get('api/groups.json').success(function(data) {
        $scope.groups = data;
      });
    };
